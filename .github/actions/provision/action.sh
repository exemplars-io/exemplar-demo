        ...
        if ($RestResponse){
          ...
          if ($MostRecentArtifactURI){
            ...
            openssl enc -d -in ./iac/state/terraform.tfstate.enc -aes-256-cbc -pbkdf2 -pass pass:"${{ inputs.encryptionkey }}" -out ./iac/terraform.tfstate
          }
        }
        terraform init
        ...
        $StateExists = Test-Path -Path ./iac/terraform.tfstate -PathType Leaf
        if ($StateExists){
          echo "state file found, encrypting with the key"
          openssl enc -in ./iac/terraform.tfstate -aes-256-cbc -pbkdf2 -pass pass:"${{ inputs.encryptionkey }}" -out ./iac/terraform.tfstate.enc
        }       